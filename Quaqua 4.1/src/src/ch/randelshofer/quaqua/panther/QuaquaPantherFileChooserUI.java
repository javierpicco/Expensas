/*
 * @(#)QuaquaPantherFileChooserUI.java  2.2  2007-10-03
 *
 ...
 * @version 2.2 2007-10-03 Added keystroke support by Bjorn Roche.
...
public class QuaquaPantherFileChooserUI extends BasicFileChooserUI {
            ...
            switch( ae.getActionCommand().charAt(0) ) {
               ...
               /* case '`':*/
                case '/':
                    ...
                    return;
                default:
                    //System.out.println( "Unknown Key Command in: " + ae );
                    break;
            }
            ...
            if (file != null) {
                ...
                for (int i=0, n = sidebarListModel.getSize(); i < n; i++) {
                    ...
                    if (sidebarFile != null && sidebarFile.getFile().equals(file)) {
                        ...
                        //System.out.println("QuaquaPantherFileChooserUI.Sidebar["+i+"]="+sidebarListModel.getElementAt(i));
                        return;
                    }
                }
    ...
    private void updateSelection() {
        ...
        /*
            ...
            //System.out.println("QuaquaPantherFileChooserUI.setSelectionPaths to "+list);
            ...
            if (! subtreeModel.isDescendant(fullPath)) {
              ...
              //  System.out.println(fullPath+" isDescendant of "+subtreeModel.getPathToRoot());
    ...
    private void doDirectoryChanged(PropertyChangeEvent e) {
        ...
        TreePath selectionPath = subtreeModel.toFullPath(browser.getSelectionPath());
        File dirForToPath = (selectedFile != null && selectedFile.isDirectory()) ?
            selectedFile : dir;
        TreePath path = model.toPath(dirForToPath, selectionPath);
        directoryComboBoxModel.setPath(path);
        
        // We only select the path in the browser, if the previously
        // selected directory was null (this is the case, when the
        // JFileChooser object was created).
        // In all other cases, the selection in the browser takes
        // precedence over what the JFileChooser object thinks.
        // This is because, JFileChooser is not really made for
        // file choosers which show a whole directory tree all the
        // times).
    ...
    protected class DirectoryComboBoxAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (chosenNode != null) {
                ...
                JFileChooser fc = getFileChooser();
                File file = chosenNode.getResolvedFile();
                if (subtreeModel.getPathToRoot().isDescendant(new TreePath(chosenNode.getPath()))) {
                    
                    // If the chosen node is visible in the browser, select it
                    if (fc.isMultiSelectionEnabled()) {
                    ...
                    // If the sidebar contains the chosen node, select it in the
                    // sidebar, and return.
                    if (sidebarList != null) {
                        ListModel sidebarModel = sidebarList.getModel();
                        for (int i=0, n = sidebarModel.getSize(); i < n; i++) {
                            FileInfo info = (FileInfo) sidebarModel.getElementAt(i);
                            if (info != null &&
                                    info.lazyGetResolvedFile().equals(file) &&
                                    sidebarList.getSelectedIndex() != i) {
                                sidebarList.setSelectedIndex(i);
                                return;
                            }
                        }
                    ...
                    // If the sidebar does not contain the chosen node,
                    // select the chosen node, and make it the root of the JBrowser
