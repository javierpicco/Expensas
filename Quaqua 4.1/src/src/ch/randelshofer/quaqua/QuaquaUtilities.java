/*
 * @(#)QuaquaUtilities.java  4.1  2007-11-10
 *
    ...
    public static final boolean isFocused(Component component) {
        ...
        if (QuaquaUtilities.isOnActiveWindow(component)) {
            ...
            if (c instanceof JComponent) {
                ...
                if (((JComponent)c).getClientProperty("Quaqua.Component.cellRendererFor") != null) {
                    c = (Component) ((JComponent)c).getClientProperty("Quaqua.Component.cellRendererFor");
                }
                ...
                // The goal of the deactivated code below, was to keep drawing a focus
                // ring on a JComponent, when the focus was temporarily moved
                // from the JComponent on a popup menu.
                // This does not work, because we also receive a temporary
                // focus loss, when the user clicks on another JFrame or
                // JDialog.
                // Unfortunately, we don't receive a permanent focusLost event,
                // on a component, after we received a non-permanent focusLost
                // event. The QuaquaFocusHandler listens on the global KeyboardFocusManager
                // and repaints components which lost permanent focus.
                /*
                Component permanentFocusOwner;
                try {
                    Object keyboardFocusManager = Methods.invokeStatic("java.awt.KeyboardFocusManager", "getCurrentKeyboardFocusManager");
                    permanentFocusOwner = (Component) Methods.invokeGetter(keyboardFocusManager, "getPermanentFocusOwner", null);
                    Object keyboardFocusManager = Methods.invokeStatic("java.awt.KeyboardFocusManager", "getCurrentKeyboardFocusManager");
                    permanentFocusOwner = (Component) Methods.invokeGetter(keyboardFocusManager, "getPermanentFocusOwner", null);
                } catch (Exception e) {
                    // Fallback for Java 1.3 or older
                    //e.printStackTrace();
                    permanentFocusOwner = null;
                }
                boolean isFocusOwner =
                        permanentFocusOwner == component ||
                        Methods.invokeGetter(c, "isFocusOwner", ((JComponent) c).hasFocus());
         * /
                ...
                return c != null
                        && (isFocusOwner || ((JComponent) c).getClientProperty("Quaqua.drawFocusBorder") == Boolean.TRUE);
            }
        }
        return false;*/
