/*
 * @(#)AliasFileSystemTreeModel.java  1.21 2007-11-16
 *
 ...
 * @version 1.21 2007-11-16 RootNode returns computer icon, if no icon
 * is available. 
...
public class AliasFileSystemTreeModel implements TreeModel {
    ...
    public void clear() {
        ...
        fireTreeNodesRemoved(this, new Object[] { root }, removedIndices, removedChildren);
    }
    ...
    public TreePath toPath(File file, TreePath templatePath) {
        ...
        for (int i=mergeIndex; i < list.size(); i++) {
            ...
            if (node.isLeaf() || node.isAlias()) break;
    ...
    public void lazyInvalidatePath(TreePath path) {
        if (DEBUG) System.out.println("lazyInvalidatePath auto="+isAutoValidate+" "+path);
        if (isAutoValidate) {
    ...
    public void validatePath(TreePath path) {
        if (DEBUG) System.out.println("AliasFileSystemTreeModel.validatPath "+path);
        for (int i=0; i < path.getPathCount(); i++) {
            Node node = (Node) path.getPathComponent(i);
                ...
                fileInfoDispatcher.dispatch(new Worker() {
                    public Object construct() {
                        ...
                        if (! doItFast) {
                            ...
                            // The Apple's FileView does not like to be used
                            // in a reentrant way.
                        ...
                        // System.out.println("info validated for "+(end-start)+" "+lazyGetResolvedFile());
                        return Boolean.FALSE;
                    }
                    public void finished(Object value) {
                        if (value == Boolean.TRUE) {
                            AliasFileSystemTreeModel.this.fireTreeNodeChanged(AliasFileSystemTreeModel.Node.this);
                        }
                        infoState = VALID;
                    }
                });
    ...
    public class DirectoryNode extends Node {
        ...
        private class DirectoryValidator implements Runnable {
            ...
            public void run() {
                ...
                //System.out.println("AliasFileSystemTreeModel fetched "+(endFetch-startTime)+" "+file);
                ...
                for (int i=0; i < freshFiles.length; i++) {
                    ...
                    if ((! isDirectoriesOnly || freshIsDirectory && freshIsTraversable)
                    && ! isHidden(freshFiles[i])) {
                ...
                //long endDetermined = System.currentTimeMillis();
                //System.out.println("AliasFileSystemTreeModel determined "+(endDetermined-endFetch)+" "+file);
                ...
                freshIndex = 0; oldIndex = 0; mergeIndex = 0;
                    ...
                    public void run() {
                        ...
                        if (newChildren.size() > 0 || deletedChildren.size() > 0) {
                            ...
                            if (newChildren.size() > 0 && deletedChildren.size() == 0) {
                                ...
                                fireTreeNodesInserted(DirectoryNode.this, getPath(), ArrayUtil.truncate(newChildIndices, 0, newChildren.size()), newChildren.toArray());
                            } else if (newChildren.size() == 0 && deletedChildren.size() > 0) {
                                children = mergedChildren;
                                fireTreeNodesRemoved(DirectoryNode.this, getPath(), ArrayUtil.truncate(deletedChildIndices, 0, deletedChildren.size()), deletedChildren.toArray());
                            } else if (newChildren.size() > 0 && deletedChildren.size() > 0) {
                        ...
                        // finished with validating.
                        Node parent = (Node) getParent();
                        if (parent != null) {
                            fireTreeNodesChanged(DirectoryNode.this, parent.getPath(), new int[] { parent.getIndex(DirectoryNode.this) }, new Object[] {DirectoryNode.this});
                        }
                        ...
                        if (DEBUG) System.out.println("AliasFileSystemTreeModel validated "+(endTime-startTime)+" "+file);
         ...
         */
        public void invalidateChildren() {
            if (DEBUG) System.out.println("AliasFileSystemTreeModel.invalidateChildren "+lazyGetResolvedFile());
        ...
        public void lazyInvalidateChildren() {
            if (validator == null && bestBeforeTimeMillis < System.currentTimeMillis()) {
                if (DEBUG) System.out.println("AliasFileSystemTreeModel.lazyInvalidateChildren YES  validator="+validator+" good for "+(bestBeforeTimeMillis - System.currentTimeMillis())+" millis "+lazyGetResolvedFile());
                childrenState = INVALID;
            } else {
                if (DEBUG) System.out.println("AliasFileSystemTreeModel.lazyInvalidateChildren NO  validator="+validator+" good for "+(bestBeforeTimeMillis - System.currentTimeMillis())+" millis "+lazyGetResolvedFile());
            }
        }
        public boolean isValidatingChildren() {
         ...
         */
        public void validateChildren() {
            if (DEBUG) System.out.println("AliasFileSystemTreeModel.validateChildren "+(childrenState)+" "+lazyGetResolvedFile());
        ...
        public TreeNode getChildAt(int childIndex) {
            if (children == null) throw new IndexOutOfBoundsException(childIndex+" >= 0");
            autoValidateChildren();
        ...
        protected File[] getFiles() {
            if (DEBUG) System.out.println("AliasFileSystemTreeModel getFiles "+lazyGetResolvedFile());
            ...
            if (resolvedFile == null && resolver == null) {
                resolver = new Worker() {
                    ...
                    public void finished(Object value) {
                        ...
                        fireTreeStructureChanged(AliasNode.this, getPath());
                    }
                };
