/*
 * @(#)QuaquaScrollPaneBorder.java  3.1.2  2007-11-03
 *
 ...
 * @version 3.1.2 2007-11-03 Fixed a class cast exception in method
    ...
    private Border getBorder(Component c) {
        ...
        if (isTextBorder(c)) {
            if (textFieldBorders == null) {
                textFieldBorders = (Border[]) QuaquaBorderFactory.create(textFieldImagesLocation, new Insets(6,6,6,6), 3, true);
            }
            borders = textFieldBorders;
        } else {
            if (scrollPaneBorders == null) {
                scrollPaneBorders = (Border[]) QuaquaBorderFactory.create(textFieldImagesLocation, new Insets(6,6,5,6), 3, true);
            }
            borders = scrollPaneBorders;
        }
        
        Component viewportView = getViewportView(c);
        if (viewportView == null) viewportView = c;
        ...
        if (QuaquaUtilities.isFocused(viewportView) ||
               (viewportView instanceof JComponent) && ((JComponent) viewportView).getClientProperty("Quaqua.drawFocusBorder") == Boolean.TRUE) {
            return borders[2];
